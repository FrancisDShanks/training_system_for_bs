...
<script>
  ...
  export default {
    ...
    methods: {
      async initData(){
        try{
          const countData = await getOrderCount({activity_id: this.activity_id});
      ...
      async getOrders(){
        const Orders = await getOrderList({offset: this.offset, limit: this.limit, activity_id: this.activity_id});
        this.tableData = [];
        Orders.forEach((item, index) => {
          ...
          tableData.activity_id = item.activity_id;
      ...
      async expand(row, status){
        if (status) {
          const restaurant = await getResturantDetail(row.activity_id);
